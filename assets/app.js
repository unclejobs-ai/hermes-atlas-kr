...
function filtered(){
  ...
  if(state.query){
    const q=state.query;
    list = list.filter(r => [r.owner,r.repo,r.name,r.description,r.descriptionKo,r.categoryKo].join(' ').toLowerCase().includes(q));
  }
  list.sort((a,b)=>{
...
function render(){
  ...
  for(const repo of list){
    ...
    btn.innerHTML = `<div class="cardTop"><div class="repoName">${escapeHtml(repo.owner)} / ${escapeHtml(repo.repo)}${repo.official?'<span class="official">공식</span>':''}</div><div class="stars">★ ${fmt(repo.stars||0)}</div></div><span class="cat">${escapeHtml(repo.categoryKo||repo.category)}</span><p class="desc">${escapeHtml(repo.descriptionKo || repo.description || '')}</p>`;
...
function renderDetail(){
  ...
  el.innerHTML = `<h3>${escapeHtml(r.owner)} / ${escapeHtml(r.repo)}</h3><p>${escapeHtml(r.descriptionKo || r.description || '')}</p><div class="detailMeta"><span class="pill">${escapeHtml(r.categoryKo || r.category)}</span><span class="pill">★ ${fmt(r.stars||0)}</span>${r.official?'<span class="pill">공식</span>':''}</div><p class="source">원문 설명: ${escapeHtml(r.description || '')}</p><a class="visit" href="${escapeAttr(r.url)}" target="_blank" rel="noreferrer">GitHub에서 보기</a>`;
}
